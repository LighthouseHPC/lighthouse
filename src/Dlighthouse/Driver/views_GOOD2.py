...
from Drivers.forms import ProblemForm, PrecisionForm, ComplexForm, MatrixTypeForm, StorageForm, SimpleForm
from Drivers.models import RoutineInfo, LinearEquation_simple, LinearEquation_expert, LinearEquation_all, LinearLeastSquare, Eigensolver 
...
app_model = {
	'Drivers':		['LinearEquation_simple', 'LinearEquation_expert', 'LinearEquation_all', 'LinearLeastSquare', 'Eigensolver',],
	...
	if form_Prob.is_valid():
 		for val in form_Prob.fields['question_prob'].choices:
			if val[0] == form_Prob.cleaned_data['question_prob']:
				...
				context = {'query_prob': val[1], 'form': form_Comp, 'results': routines}
...
def search_complex(request):
	...
	if form_Comp.is_valid():
		...
		context = {'query_prob': request.session.get('Question1')[1], 'query_comp': val_1, 'form': form_Type, 'results': routines}					
		...
		context = {'query_prob': request.session.get('Question1')[1], 'form': form_Comp, 'results': routines}
        	return render_to_response('problem.html', context_instance=RequestContext(request, context))
	...
	if form_Type.is_valid():
 		for val in form_Type.fields['question_type'].choices:
			if val[0] == form_Type.cleaned_data['question_type']:
				...
				context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'query_type': val[1], 'form': form_Stor, 'results': routines}
		...
		form_Type = MatrixTypeForm(request)
		routines = get_model(request.session.get('Application'),  request.session.get('Question1')[0]).objects.filter(**request.session['filter_dict'])
		context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'form': form_Type, 'results': routines}
		return render_to_response('complex.html', context_instance=RequestContext(request, context))
	...
	if form_Stor.is_valid():
 		for val in form_Stor.fields['question_stor'].choices:
			if val[0] == form_Stor.cleaned_data['question_stor']:
				...
				context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1], 'query_stor': val[1], 'form': form_Prec, 'results': routines}
		...
		form_Stor = StorageForm(request)
		routines = get_model(request.session.get('Application'),  request.session.get('Question1')[0]).objects.filter(**request.session['filter_dict'])
		context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1],'form': form_Stor, 'results': routines}
		return render_to_response('matrixtype.html', context_instance=RequestContext(request, context))   
...
def search_precision(request):
	...
	if form_Prec.is_valid():
		...
		routines = get_model(request.session.get('Application'),  request.session.get('Question1')[0]).objects.filter(**request.session['filter_dict'])
		context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1], 'query_stor': request.session.get('Question4')[1], 'query_prec': val_1, 'results': routines}
		...
		form_Prec = PrecisionForm()
		routines = get_model(request.session.get('Application'),  request.session.get('Question1')[0]).objects.filter(**request.session['filter_dict'])
		context = {'query_prob': request.session.get('Question1')[1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1],  'query_stor': request.session.get('Question4')[1], 'form': form_Prec, 'results': routines}
		return render_to_response('storagetype.html', context_instance=RequestContext(request, context))
...
def handle(request):
	...
	return render_to_response("handle.html", context_instance=RequestContext(request, {'form': form, 'Answers': answerlist, 'q': q}))
