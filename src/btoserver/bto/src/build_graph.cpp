...
vertex operation::to_graph(map<string, vertex>& env, map<string,type*>const& inputs, 
...
vertex variable::to_graph(map<string, vertex>& env, map<string,type*> const& inputs, 
		map<string,type*> const& outputs, graph& g)
{
...
vertex scalar_in::to_graph(map<string, vertex>& env, map<string,type*> const& inputs, 
		map<string,type*> const& outputs, graph& g)
{
  //type t(scalar, "1");
  type t;
...
void print_subgraph(std::ostream& out, subgraph* sg, graph const& g) {
  out << "subgraph cluster" << subgraph_id++ << " {\n" << std::endl;
  std::string sums = "", outs = "";
  if (sg->summations.size() > 0) {
    sums = " summing: ";
    for (unsigned int i = 0; i != sg->summations.size(); ++i)
      sums += boost::lexical_cast<string>(sg->summations[i]) + " "; 
  }
  if (sg->outputs.size() > 0) {
    outs = " outputs: ";
    for (unsigned int i = 0; i != sg->outputs.size(); ++i)
      outs += boost::lexical_cast<string>(sg->outputs[i]) + " "; 
  }
 

  out << "label = \"1.." << sg->iterations << ":" << sg->step << sums << outs 
  	<< "\\n" << sg->uid << "\";" << std::endl;
  out << "style = solid;\n";
