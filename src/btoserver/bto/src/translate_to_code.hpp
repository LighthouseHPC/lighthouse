...
void translate_graph_intrin(std::ostream& out,
							 subgraph* current,
							 vector<vertex> const& vertices,
							 vector<subgraph*> const& subgraphs,
							 graph& g);
// used to create top level temporary structures
void translate_tmp_intrin(std::ostream& out, graph &g, vertex u);
// used to declare variables and pointers
void translate_declareVariables_intrin(std::ostream& out, graph &g, vertex u, bool parallelReduce);
