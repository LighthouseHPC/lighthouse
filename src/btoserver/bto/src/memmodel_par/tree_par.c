...
//Creates a variable
struct var* create_varP(char *name, char **iterate, long long iterates, int summed){
  struct var *ret;
...
//create a loop
struct node* create_loopP(long long its, struct node **children, long long numchildren, char* iterate){
  struct node *ret;
  int i, j, k = 0;
...
//Create a statement
struct node* create_stateP(struct var **vars, long long num){
  struct node *ret;
  ret = malloc(sizeof(struct node));
  ret->its = 0;
  ret->numchildren = 0;
...
void delete_treeP(struct node* in) {
 int i;
 for(i = 0; i < in->variables; i++)
   delete_varP(in->vars[i]);
 delete_nodesP(in);
 return;
}

void delete_nodesP(struct node* in) {
  int i;
  if(in->its == 0) {
    delete_nodeP(in);
    return;
  ...
  for(i = 0; i < in->numchildren; i++)
    delete_nodesP(in->children[i]);
  delete_nodeP(in);
  return;
}
void delete_nodeP(struct node* in) {
  free(in->vars);
  free(in->iterate);
  free(in->children);
  free(in);
...
void delete_varP(struct var* in) {
  int i;
  free(in->misses);
  free(in->name);
...
/*struct node* dot_to_tree(char *infile) {
  ...
  for(i = 0; i < vars; i++) {
   ...
   childvars[i] = create_varP(name, iterate, iterates);
  }
  root = struct node* create_loopP(1, pointers to children, numchildren, iterate);
