...
long long *
all_misses (struct machine *inmachine, struct node *inloop)
{
  long long i, j, *ret;
  ret = malloc (inmachine->numcaches * (sizeof (long long)));
  for (i = 0; i < inloop->variables; i++)
    inloop->vars[i]->misses =
      malloc (inmachine->numcaches * sizeof (long long));
  for (i = 0; i < inmachine->numcaches; i++)
    for (j = 0; j < inloop->variables; j++)
      inloop->vars[j]->misses[i] = 0;
  for (i = 0; i < inmachine->numcaches; i++)
    if(strcmp(inmachine->caches[i]->name, "REG") == 0) {
      ret[i] = mem_misses_structure (inloop, inmachine->caches[i]->size, i, 1, NULL) / inmachine->caches[i]->linesize;
      //fprintf(stderr, "reg_misses: %lld\n", ret[i]);
    }
    else
      ret[i] = mem_misses_structure (inloop, inmachine->caches[i]->size, i, 0, NULL) / inmachine->caches[i]->linesize;
...
long long
mem_misses_structure (struct node *inloop, long long mem_structure_size,
		      long long structure, int regs, struct node* loop_above)
{
  ...
  for(i = 0; i < inloop->variables; i++)
    reuses[i] = 0;
  working_set_size = calc_working_set (inloop);
  ...
  if (working_set_size <= mem_structure_size)	//no misses in this loop
	...
	{
	  loop_misses +=
	    mem_misses_structure (inloop->children[i], mem_structure_size,
				  structure, regs, inloop);
//        inloop->vars[i]->misses[structure] += mem_misses_structure(inloop->children[i], mem_structure_size, structure);
	...
	{			//this loop first place for misses
	  ...
	  if(inloop->its == 0) {
	      ...
	      for (i = 0; i < inloop->variables; i++) {
	        if(first_access(inloop, i) && avail != 0 && (!iterated_above(loop_above, inloop->vars[i]->iterates, inloop->vars[i]->iterate) || !var_used_once(inloop, i))) {
                  avail--;
 		  reuses[i] = -1;
                }
                else if(find_previous_access(inloop, i) != -1){
  		  reuses[i] == -1;
		}
                else {
	  ...
	  //need something for regs in here
	    for (i = 0; i < inloop->variables; i++) {

	        ////fprintf(stderr, "l3 \n");
	      if(first_access(inloop, i)) {
	        ////fprintf(stderr, "l4 \n");
                loop_misses += wordsize;
	        ////fprintf(stderr, "l4 \n");
	        inloop->vars[i]->misses[structure] += wordsize;
	  ...
	  for (i = 0; i < inloop->variables; i++)
	    {
	       ...
	       //fprintf(stderr, "loopmisses %s %d \n", inloop->vars[j]->name, avail);
                if(first_access(inloop, j) && avail != 0 && (!var_iterated_over(inloop, j) || !var_used_once(inloop, j))) {
                  avail--;
	       //fprintf(stderr, "loopmisses %s %lld \n", inloop->vars[j]->name, inloop->vars[j]->misses[3]);
		
                  reuses[j] = -1;
                }
                else if(find_previous_access(inloop, j) != -1){
	       //fprintf(stderr, "loopmisses2 %s %lld \n", inloop->vars[j]->name, inloop->vars[j]->misses[3]);
                ...
                for(j = 0; j < inloop->variables; j++)
 	          if(reuses[j] != -1)
                    reuses[j] = calc_reuse_distance(inloop, j); 
	        avail = mem_structure_size/wordsize - 1; //how many regs minus the loop iterate
		//find_smallest(inloop->variables, inloop, avail, reuses);
	      }
	        while(reuses[i] == -1 /*&& i < inloop->numchildren*/) {
	       //fprintf(stderr, "here %s %lld \n", inloop->vars[i]->name, inloop->vars[i]->misses[0]);
	       //fprintf(stderr, "here %s %lld \n", inloop->vars[i]->name, inloop->vars[i]->misses[3]);
		  i++;}
	        if(inloop->variables == i)
		  break;
	        //fprintf(stderr, "i: %d %lld\n", i, inloop->variables);
	      if (var_used_once (inloop, i))	//variable only occurs once might be able to replace this with a call to calculate reuse distance once that function is in good form and make this logic simplier but probably not since call to RD calc only probably should be made when we need a RD
		if (var_iterated_over (inloop, i))
		  {		//if this loop iterates it only its size matters
		    loop_misses += calc_var_size (inloop, inloop->vars[i]);
		    inloop->vars[i]->misses[structure] +=
		      calc_var_size (inloop, inloop->vars[i]);
	       //fprintf(stderr, "here %s %lld \n", inloop->vars[i]->name, inloop->vars[i]->misses[3]);
		  }
		else		//if this loop doesn't this loop its count times its size if its RD is too big otherwise just its size
		if (calc_reuse_distance (inloop, i) > mem_structure_size)
		  {
//                      //fprintf(stderr, "here: %lld\n", loop_misses);
		    loop_misses +=
		      inloop->its * calc_var_size (inloop, inloop->vars[i]);
		    inloop->vars[i]->misses[structure] +=
		      inloop->its * calc_var_size (inloop, inloop->vars[i]);
		  }
		else
...
//                      //fprintf(stderr, "here2: %lld\n", loop_misses);
		    //loop_misses += calc_var_size(inloop, inloop->vars[i]); might need this back
		  }
	      else
		{		//Need to be more careful when used more than once
		  if (first_access (inloop, i))	//first access is always a special case
		    if (var_always_iterated_over (inloop, i))
			...
			else if (calc_reuse_distance (inloop, i) >
				 mem_structure_size)
			  {	//if this loop doesn't this loop its count times its size
			    loop_misses +=
			      inloop->its * calc_var_size (inloop,
							   inloop->vars[i]);
			    inloop->vars[i]->misses[structure] +=
			      inloop->its * calc_var_size (inloop,
							   inloop->vars[i]);
		    ...
		    {
		      //        //fprintf(stderr, "RD before misses: %lld\n", calc_reuse_distance(inloop, i));
		      if (calc_reuse_distance (inloop, i) > mem_structure_size)	//if doesn't fit in cache from previous miss
			if (var_iterated_over (inloop, i))
			  {	//if this loop iterates it only its size matters
			    loop_misses +=
			      calc_var_size (inloop, inloop->vars[i]);
	...
	{			//there were misses in a previous loop
	  ////fprintf(stderr, "variables: %lld\n", inloop->variables);
	   ...
	   ////fprintf(stderr, "variables: %lld\n", inloop->variables);
            ...
            find_distinct_vars(inloop, &vars);
	    ...
	    count = 0;
	    for(j = 0; j < inloop->variables; j++) 
	      if((!var_used_once(inloop, j) || !var_iterated_over(inloop, j)) && first_access(inloop, j))
		count++;
	   ...
	   ////fprintf(stderr, "misses: %lld\n", inloop->vars[j]->misses[structure]);
               if(!first_access(inloop, j) && (inloop->vars[j]->misses[structure] == wordsize || inloop->vars[j]->misses[structure] == 0)) {
	         if(avail == 0 && reuses[find_previous_access(inloop, j)] != -1) {
	    ...
	    ////fprintf(stderr, "bavail: %d\n", avail);
		 if(reuses[find_previous_access(inloop, j)] != -1) {
                   if(!in_reg[find_previous_access(inloop, j)])
		     avail--;
		   in_reg[j] = 1;
		   inloop->vars[j]->misses[structure] = 0;
		   loop_misses--;
 	//	   if(!var_iterated_over(inloop, i))  
	  ...
	  for (i = 0; i < inloop->variables; i++)
	    {
	      ...
	      if(regs == 1 && avail != 0 && !var_iterated_over(inloop, i) && reuses[i] != -1 && (inloop->vars[i]->misses[structure] == wordsize || inloop->vars[i]->misses[structure] == 0)) {
		inloop->vars[i]->misses[structure] = 0;
		loop_misses--;
	        in_reg[i] = 1;
		//if(reuses[find_next_access(inloop, i)] == -1)
                if(!in_reg[find_previous_access(inloop, i)])
		  avail--;
	  ...
	  ////fprintf(stderr, "variables2: %d\n", i);
 		    if(regs && in_reg[i])
		      ...
		      loop_misses += calc_var_size (inloop, inloop->vars[i]) - inloop->vars[i]->misses[structure];
		      inloop->vars[i]->misses[structure] =
		      calc_var_size (inloop, inloop->vars[i]);
                    }
                    else {
	  ...
	  ////fprintf(stderr, "variables2: %d\n", i);
		  ...
		  {		//if this loop doesn't this loop its count times its size         
		    ...
		    if(regs && in_reg[i])
		      continue;
                    if(inloop->vars[i]->misses[structure] == 0) {
		      loop_misses +=
		        (inloop->its-1) * calc_var_size (inloop,
							 inloop->vars[i]);
		      inloop->vars[i]->misses[structure] =
		        (inloop->its) * calc_var_size (inloop,
							 inloop->vars[i]);
	      ...
	      else
		{
		  ...
		  if (first_access (inloop, i)) {	//first access special case
		    if (var_always_iterated_over (inloop, i))
		      {		//if always iterated over just size matters
			     // //fprintf(stderr, "always iterated size %lld\n", calc_var_size(inloop, inloop->vars[i]));              
			loop_misses +=
			  calc_var_size (inloop, inloop->vars[i]) - inloop->vars[i]->misses[structure];
			inloop->vars[i]->misses[structure] =
			  calc_var_size (inloop, inloop->vars[i]);
		      }
		    else
...
//			        (calc_var_size (inloop, inloop->vars[i])*(inloop->its-1)) / inloop->its;
			      inloop->vars[i]->misses[structure] *= (inloop->its);
//			        (calc_var_size (inloop, inloop->vars[i])*(inloop->its-1)) / inloop->its;
			    }
			    else {
			  ...
			  {	//if this loop doesn't this loop its count times its size                
			    ...
			    loop_misses += (inloop->its - 1) * inloop->vars[i]->misses[structure];
// calc_var_size (inloop, inloop->vars[i]);	//minus one since we don't want ot recount previous misses
			    inloop->vars[i]->misses[structure] *= (inloop->its);
			      //(inloop->its - 1) * calc_var_size (inloop,
				//				 inloop->
				//				 vars[i]);
...
//								 vars[i]);
		    ...
		    {		//not first access
		      if(regs && in_reg[i])
		        continue;
		      var_reuse_distance = calc_reuse_distance (inloop, i);	//might be able to merge into if statement
		       ////fprintf(stderr, "var_reuse_distance: %lld\n", var_reuse_distance);
		      if (var_reuse_distance > mem_structure_size) {	//misses occur
			if (inloop->vars[i]->misses[structure] != 0)
			  {	//child has misses
			    loop_misses += (inloop->its - 1) * inloop->vars[i]->misses[structure];
//calc_var_size (inloop->children[child (inloop, i)], inloop->vars[i]);	//the misses from before are already counted
			    inloop->vars[i]->misses[structure] *= inloop->its;
			      //(inloop->its - 1) *
			      //calc_var_size (inloop->
				//	     children[child (inloop, i)],
				//	     inloop->vars[i]);
			  }
			else
...
long long
calc_reuse_distance (struct node *inloop, int location)
{
  int i, j, k, l, count = 0;
  long long reuse = 0;
  int prev, last;
  ...
  struct var **distinct;
  int numvars;
  if (var_used_once (inloop, location))
	...
	for (j = 0; j < inloop->children[i]->variables; j++)
	  if (strcmp
	    ...
	    {			//if in a subloop find that reuse distance
	      reuse = calc_reuse_distance (inloop->children[i], j);
	      if (var_iterated_over (inloop, location))	//if iterated over
		{
//                      //fprintf(stderr, "return 1 %lld\n", calc_working_set(inloop) - calc_working_set(inloop->children[i]) + reuse);
		  return calc_working_set (inloop) -
		    calc_working_set (inloop->children[i]) + reuse;
		}
	      else
		{		//find accessed in other loops
		  count = 0;
		  //          //fprintf(stderr, "return 2 %lld\n",  reuse);
		  reuse += reuse_other_loops (inloop, location);	//function to find reuse not in subloop containing the variable
		  //        //fprintf(stderr, "return 2 %lld\n",  reuse);
		  return reuse;
		}
	    }
//                      //fprintf(stderr, "return 3 %lld\n", calc_working_set(inloop));
      return calc_working_set (inloop);	//not in a subloop reuse distance = working set size
    }
  else
    {				//for both of these we need to make sure that variables are only counted once {

      ////fprintf(stderr, "else %lld\n", reuse);
      if (first_access (inloop, location))
	{			//might need to pass in something else to know this
	  ////fprintf(stderr, "else %lld\n", reuse);
	  last = find_last_access (inloop, location);
	  ////fprintf(stderr, "child RD%d\n", child(inloop, location));
	  for (i = 0; i < child (inloop, location); i++)	//add on data in loops before loop with first access
	    reuse += calc_working_set (inloop->children[i]);
	  ////fprintf(stderr, "last %lld\n", reuse);
	  for (i = inloop->numchildren - 1; i > child (inloop, last); i--)	//add on data from loops after loop with last access
	    reuse += calc_working_set (inloop->children[i]);
	  //  //fprintf(stderr, "RD %lld\n", reuse);
	  ////fprintf(stderr, "location %d\n", child_location(inloop,location));

	  reuse += before_var (inloop->children[child (inloop, location)], child_location (inloop, location));	//add on part of first access loop
	  ////fprintf(stderr, "RD %lld\n", reuse);
	  //All the below code is not needed  it doesn't matter if they're in a differnt subloop until you're in that loop
	  //if(in_different_subloop(inloop, prev, location)) {  
	  //  iterates[0] = inloop->iterate;
	  //  its[0] = inloop->its;
	  ////fprintf(stderr, "last %lld\n", last);
	  ////fprintf(stderr, "last %d\n", child_location(inloop,last));
	  //  reuse += after_var(inloop->children[child(inloop, last)], child_location(inloop, last), iterates, its, 1); //add on part of last access loop
	  //}
	  //else
          its = malloc (100 * sizeof (long long));
          iterates = malloc (100 * sizeof (char*));
	  reuse += after_var (inloop->children[child (inloop, last)], child_location (inloop, last), iterates, its, 0);	//add on part of last access loop
          free(its);
          free(iterates);
	  ////fprintf(stderr, "RD %lld\n", reuse);
	}
	...
	{			//some variables can be counted more than once in here though this may not be the case anymore
	  prev = find_previous_access (inloop, location);
	  for (i = child (inloop, prev) + 1; i < child (inloop, location); i++)	//add on data in loops between loops with accesses
	    reuse += calc_working_set (inloop->children[i]);
//        //fprintf(stderr, "RD prev %lld\n", reuse);
	  //All the below code is not needed  it doesn't matter if they're in a differnt subloop until you're in that loop
	  //  iterates[0] = inloop->iterate;
	  //its[0] = inloop->its;
	  //reuse += after_var(inloop->children[child(inloop, prev)], child_location(inloop, prev), iterates, its, 1); //add on part of last access loop
	  ...
	  ////fprintf(stderr, "prev %d\n", prev);
          its = malloc (100 * sizeof (long long));
          iterates = malloc (100 * sizeof (char*));
	  reuse += after_var (inloop->children[child (inloop, prev)], child_location (inloop, prev), iterates, its, 0);	//add on part of last access loop
          free(its);
          free(iterates);
	  ////fprintf(stderr, "RD after %lld\n", reuse);
	}
...
long long
reuse_other_loops (struct node *inloop, int location)
{
  int i, j, k, l, found;
  int count = 0;
  long long reuse = 0;
  ////fprintf(stderr, "reuse new: %lld\n", reuse);
  for (i = child (inloop, location) + 1; i < inloop->numchildren; i++)
	...
	{			//go over variables in that subloop
	  ...
	  found = 0;
	  for (k = child (inloop, location); k <= i; k++)
	  ...
	  if (found == 0)	//if the variable hasn't already been found add on its size to the reuse distance
	    reuse +=
	      calc_var_size (inloop->children[i],
			     inloop->children[i]->vars[j]);
	}
    }
  ////fprintf(stderr, "reuse new: %lld\n", reuse);
  for (i = 0; i < child (inloop, location); i++)	//loops over sub loops before the variables subloop
    for (j = 0; j < inloop->children[i]->variables; j++)
      {				//loop over their variables
	found = 0;
	////fprintf(stderr, "reuse new 10: %lld\n", reuse);
	if (!var_iterated_over (inloop, count))	//if the variable being looked at is not iterated what happened before matters
	  for (k = child (inloop, location); k < inloop->numchildren; k++)	//check all the subloops after where the variable was found
	...
	if (found == 0)		//if the variable hasn't already been found add on its size to the reuse distance
	  reuse +=
	    calc_var_size (inloop->children[i], inloop->children[i]->vars[j]);
...
long long
calc_working_set (struct node *inloop)
{
  ...
  if (inloop->its == 0)		//if I'm a statement base case
	...
	vars = find_distinct_vars (inloop, &numvars);
      ////fprintf(stderr, "numvars: %d loop\n", numvars);
      for (i = 0; i < numvars; i++)	//find working set size
	working_set_size += calc_var_size (inloop, vars[i]);
...
long long
calc_var_size (struct node *inloop, struct var *invar)
{
  long long varsize = 0;
  int i;
  long long temp;
  ...
  for (i = 0; i < inloop->numchildren; i++)
    {				//find the largest size the variable is in the tree
      ...
      temp = calc_var_size (inloop->children[i], invar);
      if (temp > varsize)
	varsize = temp;
    }
...
struct var **
find_distinct_vars (struct node *inloop, int *numvars)
{
  int i, j;
  struct var **vars;
  *numvars = 0;
...
int
var_used_once (struct node *inloop, int location)
{
  int i;
  for (i = 0; i < inloop->variables; i++)
    if (i != location)
...
int
var_iterated_over (struct node *inloop, int location)
{
  int i;
...
int
var_always_iterated_over (struct node *inloop, int location)
{
  int i, j;
  ...
  if (inloop->its == 0)
    return 1;
  if (!var_iterated_over (inloop, location))
    return 0;
  ...
  for (i = 0; i < inloop->numchildren; i++)	//figure out if the variable exists in a subloop
    for (j = 0; j < inloop->children[i]->variables; j++)
	  ...
	  (inloop->children[i]->vars[j]->name,
	   inloop->vars[location]->name) == 0)
	return (var_always_iterated_over (inloop->children[i], j));	//find out if iterated in subloop
  return 1;			//loop where variable is introduced
}

//returns 1 if the variable at location is first accessed at location
int
first_access (struct node *inloop, int location)
{
  int i;
  for (i = 0; i < inloop->variables; i++)
...
int
find_previous_access (struct node *inloop, int location)
{
  int i;
...
int
find_next_access (struct node *inloop, int location)
{
  int i;
...
int
find_last_access (struct node *inloop, int location)
{
  int i;
...
int
child (struct node *inloop, int location)
{
  int i, j = 0;
...
int
child_location (struct node *inloop, int location)
{
  int i, j, k = 0, l;
  for (i = 0; i < inloop->numchildren; i++)
    {
...
long long
before_var (struct node *inloop, int location)
{
  long long reuse = 0;
  int i;
  ...
  if (inloop->its == 0)
    return 0;
  for (i = 0; i < child (inloop, location); i++)
    reuse += calc_working_set (inloop->children[i]);
  reuse += before_var (inloop->children[child (inloop, location)], child_location (inloop, location));	//call this again with loop and location of subloop containing location
  return reuse;
}
...
long long
after_var (struct node *inloop, int location, char **iterates, long long *its,
	   int num)
{
  long long reuse = 0;
  int i;
  ...
  if (inloop->its == 0)
    {
      for (i = 0; i < inloop->variables; i++)
	reuse += calc_var_offits (inloop->vars[i], iterates, its, num);	//need to find the size of the elements accesses with our variable
      return reuse;
    }
  ////fprintf(stderr, "after var child %d %d\n", child(inloop,location), child(inloop, find_next_access(inloop, location)));
  //All the below code is not needed  it doesn't matter if they're in a differnt subloop until you're in that loop
  //if I'm used only once in this sub loop or the next access to me is in a diffent subloop
  if ( /*var_used_once(inloop, location) */ location ==
      find_last_access (inloop,
			location)
      /*|| in_different_subloop(inloop, location, find_next_access(inloop, location)) */
      )
    {
      // //fprintf(stderr, "loc: %d\n", location);
      if (!var_used_once (inloop, location))
	{
	  location = used_in_same (inloop, location);
	}
      ...
      for (i = inloop->numchildren - 1; i > child (inloop, location); i--)
	{

	  ////fprintf(stderr, "loc6: %d\n", i);
	  ////fprintf(stderr, "after var %d\n", location);
	  reuse +=
	    calc_working_set_plus_its (inloop->children[i], iterates, its,
				       num);
	  ////fprintf(stderr, "RD 1: %lld\n", reuse);
	}
    }
  reuse += after_var (inloop->children[child (inloop, location)], child_location (inloop, location), iterates, its, num);	//add on part of last access loop
  ////fprintf(stderr, "after var %d\n", location);
...
int
used_in_same (struct node *inloop, int location)
{
  int count = 0;
  int i;
  ////fprintf(stderr, "blah: \n" );
  ////fprintf(stderr, "loc2: %d\n", inloop->children[child(inloop, location)]->its);
  ////fprintf(stderr, "loc2: %d\n", inloop->numchildren);
  if (inloop->children[child (inloop, location)]->its == 0)	//if statement
    return first_access (inloop, location);
  if (inloop->numchildren == 1)	//if just one child move down the tree
    return used_in_same (inloop->children[0], location);
  if (var_used_once (inloop->children[child (inloop, location)], child_location (inloop, location)) == 1)	//if new child unique
    return location;
  location =
    used_in_same (inloop->children[child (inloop, location)],
		  child_location (inloop, location));
  for (i = 0; i < child (inloop, location); i++)
...
int
in_different_subloop (struct node *inloop, int location1, int location2)
{
  if (child (inloop, location1) != child (inloop, location2))
    return 1;
  return 0;
}
...
long long
calc_working_set_plus_its (struct node *inloop, char **iterates,
			   long long *its, int num)
{
  long long size = 0;
  int i;
  struct var **vars;
  ...
  if (inloop->its == 0)		//if I'm a statement
    for (i = 0; i < inloop->variables; i++)
      size += calc_var_offits (inloop->vars[i], iterates, its, num);	//need to find the size of the elements accesses with our variable
  else
    {				//I have sub loops
	...
	vars = find_distinct_vars (inloop, &numvars);
      for (i = 0; i < numvars; i++)	//find working set size
	size += calc_var_offits (vars[i], iterates, its, num) * calc_var_size (inloop, vars[i]);	//multiply already looked at loops by future loops
      free(vars);
    }
  return size;
}
...
long long
calc_var_offits (struct var *variable, char **iterates, long long *its,
		 int num)
{
...
void find_smallest(long long vars, struct node* inloop, int avail, long long* reuses) {
  int i, j; 
  long long smallest;
  int place;
//  //fprintf(stderr, "loop_misses: \n");
  for(i = 0; i < avail; i++) {
    ...
    for(j = 0; j < vars; j++) {
      ...
      if(place != -1)
        if(strcmp(inloop->vars[place], inloop->vars[j]) == 0 && !first_access(inloop, j)) {
	  reuses[j] = -1;
//	        //fprintf(stderr, "tis \n");
}
    }
...
int iterated_above(struct node* inloop, long long iterates, char **iterate) {
  int i;
