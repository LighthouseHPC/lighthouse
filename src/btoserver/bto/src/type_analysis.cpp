...
void compute_types(graph &g) {
	vector<vector<type> > *pt = new vector<vector<type> >(g.num_vertices(),vector<type>());
					...
					if (u.size() == 1) {
						if (pred.size() == 1) {
							if (pred[0].height != u[0].height) {
								std::cout << "transpose at " << v << ": pred mismatch 1\n";
...
void generate_constraints(graph &g) {
	...
	for (int i = 0; i != g.num_vertices(); i++) {
		...
		case output:
			...
			for (int v = 0; v != g.inv_adj(i).size(); v++) {
