...
void generatePartitionSizes(graph &g, ostream &out, int threadDepth) {
	...
	for (int i = 0; i < g.subgraphs.size(); ++i) {
		...
		if (sg->step.find("$$") == 0) {
			string tmp = string(sg->step);
			tmp.erase(0,2);
			tmp = "__s" + tmp;
			string tmp2 = string(sg->iterations);
			if (tmp2.find("$$") == 0) {
				tmp2.erase(0,2);
				tmp2 = "__s" + tmp2;
			}
...
void codeGenThreadBody(ostream& out, graph &g, subgraph *sg, int threadDepth, int curDepth) {
	...
	if (curDepth >= threadDepth) {
		vector<string> def_iters;
  		vector<string> step(10,"100");
  		vector<subgraph*> d(1,sg);
  		init_partitions(d, out, def_iters, step);
...
void codeGenThreadMessage(ostream& out, graph &g, std::set<string> &usedSizes,
	...
	// dispatch thread
	out << "pthread_create(&threads[disp], NULL, " << threadFunctionName 
		<< ", (void *)(" << messageName << "+disp));\n";
	out << "++disp;\n";
	return;
}
...
void codeGenDispatch(ostream& out, graph &g, subgraph *sg, int threadDepth, int curDepth,
	...
	string iterations = sg->iterations;
	...
	if (curDepth+1 < threadDepth) {
		...
		codeGenThreadMessage(out, g, usedSizes, data, name, 
							 boost::lexical_cast<string>(sg->uid), inOut,
							 sumtoSize);
	}
	out << "}\n";
}
			...
			int numThreads)
{
	...
	// find any top level subgraphs that are no threaded.
	vector<subgraph*> noThreadSg;
	for (int i = 0; i < g.subgraphs.size(); ++i) {
		subgraph *sg = g.subgraphs[i];
		if (sg->step.compare("1") == 0)
	...
	for (int i = 0; i != g.num_vertices(); i++) {
		...
		// temporaries amd sumtos need special casing, if they are within a thread body
		...
		if (sgDepth[g.find_parent(i)] <= threadDepth) {
			if ((g.adj(i).size() > 0 && sgDepth[g.find_parent(g.adj(i)[0])] > threadDepth) ||
				(g.inv_adj(i).size() > 0 && sgDepth[g.find_parent(g.inv_adj(i)[0])] > threadDepth)) {
	...
	for (; mesgItr != sgToMessage.end(); ++mesgItr) {
		...
		for (; im != mesgItr->second.end(); ++im) {
			...
			if (g.info(im->second).op == sumto) { 
				subgraph *p = g.find_parent(g.inv_adj(im->second)[0]);
				bool threaded = true;
				while (p) {
	...
	for (unsigned int i = 0; i < g.subgraphs.size(); ++i) {
		...
		if (g.subgraphs[i]->step.compare("1") == 0)
			continue;
		...
		out << "} " << name << "_" << boost::lexical_cast<string>(sg->uid) 
			<< "_msg_t;\n\n";
	}
	...
	for (unsigned int i = 0; i < g.subgraphs.size(); ++i) {
		...
		subgraph *sg = g.subgraphs[i];
		string uid = boost::lexical_cast<string>(sg->uid);
		string messageType = name + "_" + uid + "_msg_t";
		string threadFunctionName = name + "_body_" + uid;
	...
	generatePartitionSizes(g,out,threadDepth);

	
	// create thread and message structures
	out << "\n// threads and message structures\n";
	out << "pthread_t *threads = (pthread_t*)malloc(sizeof(pthread_t)*nparts);\n";
	for (unsigned int i = 0; i < g.subgraphs.size(); ++i) {
		// not all top level subgraphs are partitioned for parallel
		if (g.subgraphs[i]->step.compare("1") == 0)
			continue;
	...
	for (unsigned int i = 0; i != order.size(); ++i) {
		...
		// clean up
		out << "free(mesg_" << boost::lexical_cast<string>(sg->uid) << ");\n";
	}
	out << "free(threads);\n";
