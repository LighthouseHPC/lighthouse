...
#"request.session" is a dictionary that records the answers: request.session = {'Routines': {}, Routines_one: [], 'Question1': [], 'Question2': [], ...}
...
def search_problem(request):
	...
	if form_Prob.is_valid():
		...
		if selected == [u'LinearEquation']:
			routines_one = list(chain(LinearEquation_expert.objects.filter(notes__icontains='TRANS'), LinearEquation_simple.objects.all()))
			form = EquationForm()
			...
			if len(selected) == 1:
				routines_one = LinearEquation_computational.objects.filter(notes__icontains=selected[0])
				action = '/search/problem/complex/'
				request.session['Question1.5']=[0, 0]
				form = ComplexForm()
			else:	
				for item in selected:
					Routines[item] = LinearEquation_computational.objects.filter(notes__icontains=item)
				action = '/search-form2/'			
				form = SimpleForm()

		else:
			for item in selected:
				if item == 'LinearEquation':
					Routines['trans'] = LinearEquation_expert.objects.filter(notes__icontains='TRANS')
				elif item == 'solve' or 'inverse':
					Routines[item] = LinearEquation_computational.objects.filter(notes__icontains=item)
				else:
					Routines[item] = LinearEquation_expert.objects.filter(notes__icontains=item)

			if len(selected) == 2 and 'solve' not in selected or 'inverse' not in selected:	
		...
		request.session['Routines_one'] = routines_one	

		context = {'query_prob': Answers, 'form': form, 'Action': action, 'results': routines_one}
...
def search_equation(request):
	...
	if form_Equa.is_valid():
		if form_Equa.cleaned_data['question_equa'] == unicode('transpose'):
			val_0 = 'transpose'
			val_1 = 'A**T*X = B'
			routines = request.session['Routines_one'].filter(Q(thePrecision='s')|Q(thePrecision='d'))
			initial_value = 'n'

		elif form_Equa.cleaned_data['question_equa'] == unicode('Hermitian_trans'):
			val_0 = 'Hermitian_trans'
			val_1 = 'A**H*X = B'
			routines = request.session['Routines_one'].filter(Q(thePrecision='c')|Q(thePrecision='z'))
			initial_value = 'y'	
		...
		request.session['Routines_one'] = routines
		request.session['Question1.5'] = [val_0, val_1] 
		form = ComplexForm(initial=dict(question_comp=initial_value))

		context = {'query_prob': request.session['Question1'], 'query_equa': val_1, 'form': form, 'results': routines}					
...
def search_complex(request):
	...
	if form_Comp.is_valid():
		if form_Comp.cleaned_data['question_comp'] == unicode('y'):
			val_0 = 'y'
			val_1 = 'Yes'
			request.session['Routines_one'] = request.session['Routines_one'].filter(**{'thePrecision__in': ['c', 'z']})

			
		else:
			val_0 = 'n'
			val_1 = 'No'
			request.session['Routines_one'] = request.session['Routines_one'].filter(**{'thePrecision__in': ['s', 'd']})
			
		request.session['Question2'] = [val_0, val_1] 
		form = MatrixTypeForm(request)	
		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': val_1, 'form': form, 'results': request.session['Routines_one']}					
		...
		return render_to_response('complex.html', context_instance=RequestContext(request, context))
        	...
        	form = ComplexForm()
		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'form': form, 'results': request.session['Routines_one']}
        	return render_to_response('problem.html', context_instance=RequestContext(request, context))
	...
	if form_Type.is_valid():
 		for val in form_Type.fields['question_type'].choices:
			if val[0] == form_Type.cleaned_data['question_type']:
				request.session['Routines_one'] = request.session['Routines_one'].filter(matrixType = val[0])
				request.session['Question3'] = [val[0], val[1]]
				form = StructureForm(request)
				context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'query_type': val[1], 'form': form, 'results': request.session['Routines_one']}
				return render_to_response('matrixtype.html', context_instance=RequestContext(request, context)) 

	
	else:
		form = MatrixTypeForm(request)
		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'form': form, 'results': request.session['Routines_one']}
		return render_to_response('complex.html', context_instance=RequestContext(request, context))
	...
	if form_Stor.is_valid():
 		for val in form_Stor.fields['question_stor'].choices:
			if val[0] == form_Stor.cleaned_data['question_stor']:
				request.session['Routines_one'] = request.session['Routines_one'].filter(structureType = val[0])
				request.session['Question4'] = [val[0], val[1]]
				form = PrecisionForm()
				context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1], 'query_stor': val[1], 'form': form, 'results': request.session['Routines_one']}
				return render_to_response('structuretype.html', context_instance=RequestContext(request, context)) 

	
	else:
		form = StructureForm(request)
		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1],'form': form_Stor, 'results': request.session['Routines_one']}
		return render_to_response('matrixtype.html', context_instance=RequestContext(request, context))   
	...
	if form_Prec.is_valid():
		if form_Prec.cleaned_data['question_prec'] == unicode('d'):
			...
			if request.session.get('Question2')[0] == 'y':
				request.session['Routines_one'] = request.session['Routines_one'].filter(thePrecision = 'z')
			
			if request.session.get('Question2')[0] == 'n':
				request.session['Routines_one'] = request.session['Routines_one'].filter(thePrecision = 'd')

		else:
			val_0 = 's'	
			val_1 = 'Single'
			if request.session.get('Question2')[0] == 'y':
				request.session['Routines_one'] = request.session['Routines_one'].filter(thePrecision = 'c')
			
			if request.session.get('Question2')[0] == 'n':
				request.session['Routines_one'] = request.session['Routines_one'].filter(thePrecision = 's')	

		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1], 'query_stor': request.session.get('Question4')[1], 'query_prec': val_1, 'results': request.session['Routines_one']}
		return render_to_response('precision.html', context_instance=RequestContext(request, context)) 		


	else:
		form = PrecisionForm()
		context = {'query_prob': request.session['Question1'], 'query_equa': request.session['Question1.5'][1], 'query_comp': request.session.get('Question2')[1], 'query_type': request.session.get('Question3')[1],  'query_stor': request.session.get('Question4')[1], 'form': form, 'results': request.session['Routines_one']}
		return render_to_response('structuretype.html', context_instance=RequestContext(request, context))
