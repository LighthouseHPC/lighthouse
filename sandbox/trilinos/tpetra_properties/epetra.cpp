...
int main(int argc, char *argv[]) {
  ...
  if (MyPID == 0) {
    cout << "K dims:" << K->NumGlobalRows() << ", " << K->NumGlobalCols() << endl;
    cout << "M dims:" << M->NumGlobalRows() << ", " << M->NumGlobalCols() << endl;
  }
  ...
  Teuchos::RCP<Ifpack_Preconditioner> Prec =
     Teuchos::rcp( Factory.Create(PrecType, K.get(), OverlapLevel) );
