...
def guidedSearch_index(request):
    ...
    #request.session['Results'] = str(lapack_least_guided.objects.all())
    request.session['Results'] = str(lapack_least_guided.objects.all())
    ...
    if form.is_valid():
        ...
        if value == 'stop':
            ...
            ## do search based on user's response
                    ...
                    query = {lookup : value}
                    #request.session['Results'] = request.session['Results'].objects.filter(**query)
                    request.session['Results'] = request.session['Results'].objects.all()
