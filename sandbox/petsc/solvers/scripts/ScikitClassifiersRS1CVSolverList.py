#This file does classification using scikit for reduced features: Gaussian NaiveBayes, Decision Tree, RF
...
#8 features in RS1
filename = '../PETSc_FullFeatureSet/petsc_anamodRS1_35.arff'
...
gnb_predictn = cross_val_predict(classifierNB,X1,target, cv = 10)
...
test_file = '../PETSc_FullFeatureSet/TestInstance.arff'
...
ctr = 0
testset1.resize(154,9)
while ctr <=153:
	for i in range(0,153):
	
		testset1[i+1,:] = testset1[0,:]
		testset1[i,8] = solvers[0][i]
		ctr += 1
		out[i]= "good"
print("out --", out.shape,testset1.shape)
#now to predict good solvers,train on whole data
rf = RandomForestClassifier(n_estimators = 100, n_jobs = 12, random_state = 4)
rf.fit(X1,target)
dtree.fit(X1,target)
...
for i in range(res_RF.shape[0]):
	if res_RF[i].decode('UTF-8') == "good" : 
		good_solvers.append(int(testset1[i,8]))
	elif res_RF[i].decode('UTF-8') == "bad" : 
		bad_solvers.append(int(testset1[i,8]))
...
for i in range(res_DT.shape[0]):
	if res_DT[i].decode('UTF-8') == "good" : 
		good_solvers_DT.append(int(testset1[i,8]))
	elif res_DT[i].decode('UTF-8') == "bad" : 
		bad_solvers_DT.append(int(testset1[i,8]))
...
for i in range(res_SVM.shape[0]):
	if res_SVM[i].decode('UTF-8') == "good" : 
		good_solvers_SVM.append(int(testset1[i,8]))
	elif res_SVM[i].decode('UTF-8') == "bad" : 
		bad_solvers_SVM.append(int(testset1[i,8]))
...
for i in range(res_knn.shape[0]):
	if res_knn[i].decode('UTF-8') == "good" : 
		good_solvers_knn.append(int(testset1[i,8]))
	elif res_knn[i].decode('UTF-8') == "bad" : 
		bad_solvers_knn.append(int(testset1[i,8]))
