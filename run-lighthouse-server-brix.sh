...
if [ "x$1" != "x" ]; then port=$1; else port=8086; fi
...
pythonexec=/home/norris/soft/djangostack-1.5.8-0/python/bin/python
...
#cd src/Dlighthouse  && $pythonexec manage.py rebuild_index <<< $(echo 'Y') &&   
cd src/Dlighthouse  &&  
$pythonexec manage.py runserver brix.d.cs.uoregon.edu:$port || echo  "FAIL: Could not start the Lighthouse server"
